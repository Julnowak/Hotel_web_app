...
const OneRoomVisual = ({hotelId, floorId}) => {
    ...
    const fetchRooms = async () => {
        try {
            const response = await client.get(`${API_BASE_URL}/rooms/`,
                {
                    params: {
                        hotel_id: hotelId,
                        floor: floorId
            ...
            <div className="hotel-container">
                ...
                <div className="rooms-top">
                    {rooms ? rooms.slice(5, 15).map((room) => (
                        <div
                            ...
                            ${params.id.toString() === room.room_number.toString() ? 'wybrany' : 'niewybrany'}
                            `}
                        >
                ...
                <div className="center-section">
                    ...
                    <div className="rooms-left">
                        {rooms ? rooms.slice(0, 5).reverse().map((room) => (
                            <div
                                key={room.room_number}
                                className={`roomStatus 
                            ${params.id.toString() === room.room_number.toString() ? 'zajęty' : 'niewybrany'}
                            `}
                            >
                                <b>{room.room_number}</b>
                            </div>
